...
export default function ({favorite, setFavorite, newFavorite, setNewFavorite, statusModal}) {
  ...
  const {checkboxSelecionado, setCheckboxSelecionado} = useContext(MenuContext)
    ...
    return (
        <>
<input
  class="mr-2 mt-[0.3rem] h-3.5 w-8 appearance-none rounded-[0.4375rem] bg-neutral-300 before:pointer-events-none before:absolute 
  before:h-3.5 before:w-3.5 before:rounded-full before:bg-transparent before:content-[''] after:absolute after:z-[2] after:-mt-[0.1875rem] 
  after:h-5 after:w-5 after:rounded-full after:border-none after:bg-green_button after:shadow-[0_0px_3px_0_rgb(0_0_0_/_7%),_0_2px_2px_0_rgb(0_0_0_/_4%)] 
  ...
  checked:after:transition-[background-color_0.2s,transform_0.2s] checked:after:content-[''] hover:cursor-pointer focus:outline-none focus:ring-0 focus:before:scale-100
  ...
  checked={statusModal? favorite : newFavorite}
  onChange={statusModal? () => setFavorite(!favorite) : () => setNewFavorite(!newFavorite)}
  />
      </>
